    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="公告类型" prop="gonggaoleixing">
          <el-select v-model="ruleForm.gonggaoleixing" placeholder="请选择公告类型">
            <el-option
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gonggaoleixing/gonggaoleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.gonggaoleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.gonggaoneirong = this.ruleForm.gonggaoneirong.replace(reg,'../../../django5hna8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shangchenggonggao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangchenggonggaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
